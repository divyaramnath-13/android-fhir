...
import com.google.android.fhir.demo.external.FingernailsPhotoCaptureUtils
import com.google.android.material.snackbar.Snackbar
import java.util.UUID
import org.hl7.fhir.r4.model.Coding
import org.hl7.fhir.r4.model.QuestionnaireResponse

object FingernailsPhotoSensorCaptureFactory :
  ...
  override fun getQuestionnaireItemViewHolderDelegate() =
    object : QuestionnaireItemViewHolderDelegate {
      ...
      override fun init(itemView: View) {
        ...
        capturePhotoButton.setText("Capture Fingernails Photo")
      ...
      private fun onTakePhotoButtonClicked(view: View/*, questionnaireItem: Questionnaire.QuestionnaireItemComponent*/) {
        val status = FingernailsPhotoCaptureUtils.capturePhoto(captureId)
        if (status) {
          ...
          val answer = QuestionnaireResponse.QuestionnaireResponseItemAnswerComponent().apply {
            value = Coding().apply {
              code = captureId
              system = "FingernailsPhotoSensorCaptureFactory"
            }
          }
          questionnaireViewItem.setAnswer(answer)

          val photoFilePath = FingernailsPhotoCaptureUtils.getPhotoPath(captureId)
          textView.text = "Photo captured at: " + photoFilePath
        }
      }
    }
  const val WIDGET_EXTENSION = "http://external-api-call/sensing-backbone"
  const val WIDGET_TYPE = "photo-fingernails-capture"
}