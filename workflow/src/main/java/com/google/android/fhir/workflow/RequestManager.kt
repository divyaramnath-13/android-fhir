...
class RequestManager(
  ...
  suspend fun getRequestsForPatient(
    ...
    return if (enumValues<SupportedRequestResources>().any { it.value.toCode() == requestType.name }) {
      ...
      fhirEngine.search(xFhirQueryBuilder.toString())
    } else
      null  // not a valid or supported request
  }
  ...
  suspend fun getAllRequestsForPatient(patientId: String,
    ...
    for (requestResourceType in SupportedRequestResources.values()) {
      ...
      requestList.addAll(fhirEngine.search(xFhirQueryBuilder.toString()))
    }
    return requestList
  }
